...
namespace ns_control
{
    ...
    public:
        ...
        std::mutex *mtx; // mutex是禁止拷贝的，使用指针
    public:
    ...
    public:
        ...
        void Judge(const std::string &number, const std::string &in_json, std::string *out_json)
        {
            ...
            while (true)
            {
                ...
                m->IncLoad();
                LOG(INFO) << " 选择主机成功，主机id：" << " 详情：" << m->ip << ":" << m->port << " 当前主机的负载是：" << m->Load() << "\n";
                ...
                {
                    //请求失败
                    LOG(ERROR) << "当前的主机id：" << id << "详情：" << m->ip << ":" << m->port << " 可能已经离线" << "\n";
                    load_balance_.OfflineMachine(id);
                    load_balance_.ShowMachines();//仅用来调试
                }
